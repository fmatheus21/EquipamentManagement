 ...
 * {@link com.mycompany.myapp.domain.StoreEquipment}.
 */
@RestController
@RequestMapping("/api")
public class StoreEquipmentResource {
     ...
     * with body the new storeEquipment, or with status
     * {@code 400 (Bad Request)} if the storeEquipment has already an ID.
     ...
     * body the updated storeEquipment, or with status {@code 400 (Bad Request)}
     * if the storeEquipment is not valid, or with status
     * {@code 500 (Internal Server Error)} if the storeEquipment couldn't be
     * updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
     ...
     * list of storeEquipments in body.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with
     * body the storeEquipment, or with status {@code 404 (Not Found)}.
     */
    ...
    @GetMapping("/store-equipments/store/{id}")
    public ResponseEntity<List<StoreEquipment>> getStoreEquipmentByStore(@PathVariable Long id) {
        List<StoreEquipment> storeEquipments = storeEquipmentService.findByStore(id);
        return !storeEquipments.isEmpty() ? ResponseEntity.ok().body(storeEquipments) : ResponseEntity.status(204).build();
